...
@Service
public class GeminiService implements  PlatService {
    ...
    public CoinPlatModel getTicker(CoinPlatModel coinPlatModel) {
        ...
        String url = "https://api.gemini.com/v1/pubticker/";
        ...
        JSONObject vol=apiBack.getJSONObject("volume");
        String coin=symbol.substring(0,3).toUpperCase();
        newCoinPlat.setTradingTime(new Date(vol.getLong("timestamp")));
        logger.info("coin "+coin);

        newCoinPlat.setVol(new BigDecimal(vol.getString(coin)));
        return newCoinPlat;
    }
